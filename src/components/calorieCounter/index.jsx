...
const GOAL_KEY = "calorie-counter.goal";
...
const loadGoal = () => {
    const val = Number(localStorage.getItem(GOAL_KEY));
    return Number.isFinite(val) && val > 0 ? val : 2000; // sensible default
};
...
export default function CalorieCounter() {
    ...
    const mealsForDay = useMemo(() => {
        ...
        // sort: mealType order -> time added desc
    ...
    const grouped = useMemo(() => {
        ...
        return Array.from(map.entries()); // [ [mealType, items[]], ... ]
    }, [mealsForDay]);
    ...
    const pctOfGoal =
        goal > 0 ? Math.min(999, Math.round((totalKcal / goal) * 100)) : 0;
    ...
    const clearDay = () => {
        setMeals((prev) => prev.filter((m) => m.date !== selectedDate));
    };
            ...
            <Styled.Container>
                <Styled.Header>
                    <div>
                        ...
                        <Styled.Sub>
                            Log meals • Track daily calories • LocalStorage
                        </Styled.Sub>
                    </div>

                    <div style={{ textAlign: "right" }}>
                        <Styled.Badge>{totalKcal} kcal today</Styled.Badge>{" "}
                        <Styled.Badge tone="muted">
                            Goal: {goal} kcal ({pctOfGoal}%)
                        </Styled.Badge>
                        <div style={{ fontSize: 12, opacity: 0.8, marginTop: 6 }}>
                            {over > 0 ? `Over by ${over} kcal` : `Remaining ${remaining} kcal`}
                        </div>
                    </div>
                </Styled.Header>
                ...
                <Styled.Card as="form" onSubmit={addMeal}>
                    <Styled.FormRow>
                        ...
                        <Styled.Input
                            ...
                            aria-label="Log date"
                        />
                        <Styled.PrimaryButton type="submit" disabled={!name.trim() || !Number(kcal)}>
                            Add
                        </Styled.PrimaryButton>
                    </Styled.FormRow>
                    {(!name.trim() || !Number(kcal)) && (
                        <Styled.Helper>Tip: Enter meal name and calories.</Styled.Helper>
                    )}
                </Styled.Card>
                ...
                <Styled.Toolbar>
                    <Styled.RowWrap>
                        <Styled.Button type="button" onClick={() => shiftDay(-1)}>
                            ◀ Prev
                        </Styled.Button>
                        <Styled.Input
                            type="date"
                            ...
                            style={{ width: 160 }}
                        />
                        <Styled.Button type="button" onClick={() => setSelectedDate(todayISO())}>
                            Today
                        </Styled.Button>
                        <Styled.Button type="button" onClick={() => shiftDay(1)}>
                            Next ▶
                ...
                <Styled.List>
                    {mealsForDay.length === 0 && (
                        <Styled.Empty>No meals logged on {fmtDateNice(selectedDate)}.</Styled.Empty>
                    )}
                                        ...
                                        <Styled.ItemRight>
                                            ...
                                            <Styled.IconButton
                                                onClick={() =>
                                                    askConfirm({
                                                        title: "Delete meal?",
                                                        message: `Delete “${m.name}” (${m.kcal} kcal)?`,
                                                        confirmText: "Delete",
                                                        tone: "danger",
